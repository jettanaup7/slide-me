...
function Chat({ setIsChatOpen, orderID, token }) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      fetchChats(); // Fetch every 5 seconds
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(`http://localhost:3000/chats/add`, {
      ...
      <div className="chat-main-container">
        {chats.map((chat, index) => {
          ...
          return (
            <div
              key={index}
              className={senderType === "driver" ? "outgoing-message" : "incoming-message"}
            >
              {senderType !== "driver" && (
              ...
              <div className={`bubble ${senderType === "driver" ? "outgoing" : "incoming"}`}>
                {chat.isPicture ? (
                  <img
                    src={chat.message}
                    alt="Image message"
